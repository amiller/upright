...
tmux new-session    "sleep 2; bash -c 'java ${JAVA_OPTS} BFT.order.OrderBaseNode 0 config.properties | tee order0.log'; bash" \;  \
    ...
    splitw -h -p 50 'java ${JAVA_OPTS} BFT.filter.FilterBaseNode 0 config.properties' \; \
    selectp -t 2 \; \
    ...
    selectp -t 5 \; \
    splitw -v -p 50 'sleep 5; for i in `seq 20`; do java ${JAVA_OPTS} Applications.hashtable.HTClient config.properties 0 | tee -a client0.log; sleep 1; done; bash' \; \
    selectp -t 0 \; \
