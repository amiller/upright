  ...
  private static ClientProtocol createRPCNamenode(InetSocketAddress nameNodeAddr,
    ...
    throws IOException {
  	...
  	if(conf.getBoolean("dfs.bft", false)){
  		...
  		BftClientWrapper bftClientWrapper = 
  			new BftClientWrapper(BFTRequest.ProtocolType.CLIENT, conf);
		...
		*/
  		...
  		BftClientGlue bcg = new BftClientGlue(NodeType.CLIENT, conf);
  		//try {
			//bcg.initialize(UnixUserGroupInformation.login(conf, true));
  		LOG.debug("intializing glue");
	...
	//	}
  		...
  		return (ClientProtocol)RPC.getProxy(ClientProtocol.class, 
  				ClientProtocol.versionID, wrapperAddr,
