...
public class BFTBlockReceiver extends BlockReceiver {
	...
	public static final int bytesPerMD = 64*1024; // 64K
  ...
  //private LinkedList<MD5Hash> md5list;
    ...
    try {
		...
		//md5list = new LinkedList<MD5Hash>();
	}
  ...
  protected int receivePacket() throws IOException {
    ...
    if (len == 0) {
      ...
      try {
        if (!finalized) {
          ...
          /*
          //caculate md5 of data in this packet
          try{
          	MD5Hash md5 = MD5Hash.digest(pktBuf,dataOff, len);
          	LOG.debug("BFT : Packet Seq no : " + seqno +
          			", dataLen="+len+ "\nDigest : " + md5.toString());

          	mdOut.writeInt(md5.getDigest().length);
          	mdOut.write(md5.getDigest());
    ...
    if(lastPacketInBlock){
      // calculate md5 of hashes in md5list
      ...
      LOG.debug("ADDED Hash Val : " + (addedDigest==null?"None":new MD5Hash(addedDigest).toString()));
    	...
    	md5list.clear();
    	LOG.debug("FINAL from list : " + new MD5Hash(digester.digest()).toString());
    	*/
    }
