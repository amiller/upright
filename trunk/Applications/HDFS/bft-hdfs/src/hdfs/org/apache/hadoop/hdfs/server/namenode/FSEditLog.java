...
public class FSEditLog {
	protected static final byte OP_INVALID = -1;
	...
	static int loadFSEdits(EditLogInputStream edits) throws IOException {
		...
		if(FSNamesystem.bft) return BftFSEditLog.loadFSEdits(edits);
	...
	public void logCloseFile(String path, INodeFile newNode) {
		String t_path;
		if(FSNamesystem.log_fault_inject){
			Random r = new Random();
			if(r.nextInt(100) > 50){
				t_path = path + "a";
				System.out.println("Fault is injected for " + path);
			}else{
				t_path = path;
				System.out.println("Fault is not injected for " + path);
			}
	...
	protected static UTF8 toLogReplication(short replication) {
		return new UTF8(Short.toString(replication));
	}

	protected static UTF8 toLogLong(long timestamp) {
	...
	synchronized ArrayList<EditLogOutputStream> 
	swapEditLog() throws IOException {
		...
		ArrayList<EditLogOutputStream> ret = editStreams;
		String curLogName = ret.get(0).getName();
		String nextLogFileName;
		int nextLogNum = -1;
		int logNumIdx = curLogName.lastIndexOf('_');
		if(logNumIdx < 0){
			nextLogNum = 1;
			nextLogFileName = curLogName + "_" + nextLogNum;
		} else {
			nextLogNum = Integer.parseInt(curLogName.substring(logNumIdx+1)) +1;
			nextLogFileName = curLogName.substring(0, logNumIdx) + "_" + nextLogNum;
		}
		...
		for (Iterator<StorageDirectory> it = 
			...
			StorageDirectory sd = it.next();
			try {
				File newLogFile = new File(nextLogFileName);
				//newLogFile.createNewFile();
				EditLogFileOutputStream eStream = 
					new EditLogFileOutputStream(newLogFile);
				eStream.create();
				editStreams.add(eStream);
			} catch (IOException e) {
				e.printStackTrace();
	...
	synchronized ArrayList<EditLogOutputStream> 
	swapEditLogTo(String fileName) throws IOException {
	...
	protected static short readShort(DataInputStream in) throws IOException {
