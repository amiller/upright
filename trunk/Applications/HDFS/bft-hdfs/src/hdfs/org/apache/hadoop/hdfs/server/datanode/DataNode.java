...
import org.apache.hadoop.hdfs.BftClientGlue;
import org.apache.hadoop.hdfs.BftClientWrapper;
...
public class DataNode extends Configured 
  ...
  void startDataNode(Configuration conf, 
    ...
    fakemd5 = conf.getBoolean("dfs.bft.fakemd5", false);
    if(conf.getBoolean("dfs.bft", false)){
  		LOG.info("BFT Mode");
  		bft = true;
  		/*
  		BftClientWrapper bftClientWrapper = 
  			new BftClientWrapper(BFTRequest.ProtocolType.DATANODE, conf);
  		
  		try {
  			bftClientWrapper.initialize(UnixUserGroupInformation.login(conf, true));
  		...
  		InetSocketAddress wrapperAddr = bftClientWrapper.getRPCServerAddr();
  		*/
  		
  		BftClientGlue bcg = new BftClientGlue(NodeType.DATANODE, conf);
  		try {
			bcg.initialize(UnixUserGroupInformation.login(conf, true));
		} catch (LoginException e) {
			e.printStackTrace();
			System.err.println(e.getMessage());
			System.exit(-1);
		}
  		InetSocketAddress wrapperAddr = bcg.getRPCServerAddr();

  		this.namenode = (DatanodeProtocol)RPC.waitForProxy(DatanodeProtocol.class, 
  				DatanodeProtocol.versionID, wrapperAddr,
  				conf);
