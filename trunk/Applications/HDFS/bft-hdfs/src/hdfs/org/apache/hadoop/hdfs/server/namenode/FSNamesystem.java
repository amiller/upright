...
public class FSNamesystem implements FSConstants, FSNamesystemMBean {
  ...
  /*
   * FSNameSystem Constructor used by CPHelper
   */
  
  FSNamesystem(Configuration conf, boolean doFormat) throws IOException {
  	bft = conf.getBoolean("dfs.bft", false);
  	bftdatanode = conf.getBoolean("dfs.bft.datanode", false);
    try {
      initialize(conf, doFormat);
    } catch(IOException e) {
      LOG.error(getClass().getSimpleName() + " initialization failed.", e);
      close();
      throw e;
  ...
  /*
   * Initialization function used by CPHelper
   */
  private void initialize(Configuration conf, boolean doFormat) throws IOException {
    ...
    this.dir = new FSDirectory(new FSImage(getNamespaceDirs(conf),getNamespaceEditsDirs(conf)), this, conf);
    StartupOption startOpt = StartupOption.REGULAR;
    Collection<File> chkpointdir = getNamespaceDirs(conf);
    if(doFormat){
    	this.dir.fsImage.format();
    }
    ...
    if (iFile != null && iFile.isUnderConstruction()) {
  	...
  	synchronized(undecidedBlocks){
  		if( undecidedBlocks.containsKey(previousBlock)){
  			...
  			HashMap<byte[],Integer> quorumCheck = new HashMap<byte[],Integer>();
  			...
  			for(Object[] triplet : tripletList){
  				...
  				if(Arrays.equals(hashOfPreviousBlock, hash)){
  					...
  					if(quorumCheck.containsKey(b.getHash())){
  						int prev = quorumCheck.get(b.getHash());
  						quorumCheck.put(b.getHash(), prev+1);
  					} else {
  						quorumCheck.put(b.getHash(), 1);
  					}  					
  				} else {
  			...
  			int max = 0;
  			byte[] maxHash = null;
  			for(byte[] hash : quorumCheck.keySet()){
    	...
    	if ( hashFromClient == null ){
    		...
    		synchronized(undecidedBlocks){
    			...
    			if(tripletList.size() < this.minReplication){
    				...
    				HashMap<byte[],Integer> quorumCheck =
      			...
      			int max = 0;
      			byte[] maxHash = null;
      			for(byte[] hash_ : quorumCheck.keySet()){
      			...
      			if(max < this.minReplication){
      				...
      				blocksMap.getStoredBlock(block).setHash(maxHash);
  ...
  class SafeModeInfo implements Writable {
    ...
    public void write(DataOutput out) throws IOException {
      ...
      System.out.println("et:"+extension);
      System.out.println("sr:"+safeReplication);
      System.out.println("re"+reached);
      System.out.println("bt:"+blockTotal);
      System.out.println("bs:"+blockSafe);
      System.out.println("lsr:"+lastStatusReport);
      */
    }
  }
  ...
  ////////////////////////////////////////////////////////////////////////
  // The following saveXXX()/readXXX() are added to save the extra states
  // into a checkpoint in order to support bft namenode

  void saveRecentInvalidateSets(DataOutputStream out) throws IOException {
  	//Text.writeString(out, "saveRecentInval");
  	//System.out.println("saveRecentInvalidateSets");
  	...
  	for (String sid : recentInvalidateSets.keySet()){
  		...
  		//System.out.println("## Saving - SID : " + sid);
  		...
  		for (Block b : blocks){
  			...
  			//System.out.println("## Saving - blockId : " + b.getBlockId());
  			//
  ...
  void readRecentInvalidateSets(DataInputStream in) throws IOException {
    ...
    for(int i=0; i<size; i++){
      ...
      System.out.println("## Loading - SID : " + sid);
      ...
      for(int j=0; j<nBlocks; j++){
      	...
      	b.readFields(in);
      	blocks.add(b);
      	//System.out.println("## Loading - blockId : " + b.getBlockId());
      }
  ...
  void saveExcessReplicateMap(DataOutputStream out) throws IOException {
    ...
    for (String sid : excessReplicateMap.keySet()){
      ...
      for (Block b : blocks){
        ...
        //	System.out.println("Block "+b.getBlockId()+" is not in blocksMap");
        //}
      }      
    }
  ...
  void readExcessReplicateMap(DataInputStream in) throws IOException {
    ...
    for(int i=0; i<size; i++){
      ...
      //System.out.println("#*# Loading - SID : " + sid);
      int nBlocks = in.readInt();
      //System.out.println("size of blocks :" + nBlocks);
      Collection<Block> blocks  = new TreeSet<Block>();
      for(int j=0; j<nBlocks; j++){
      	Block b = new Block(in.readLong());
      	//System.out.println("#*# Reading - blockId : " + b.getBlockId());
      	//if (blocksMap.getStoredBlock(b)==null){
      	//	System.out.println("Block "+b.getBlockId() +"was not found in blocksMap");
      	//}
  ...
  void saveHeartbeats(DataOutputStream out) throws IOException {
    ...
    while(iter.hasNext()){
      ...
      //System.out.println("## Saving - SID : " + dd.getStorageID());
    }
  ...
  void saveNeededAndPendingReplication(DataOutputStream out) throws IOException {
    ...
    /*
    System.out.println("ct:"+capacityTotal);
    System.out.println("cu:"+capacityUsed);
    System.out.println("cr:"+capacityRemaining);
    System.out.println("tl:"+totalLoad);
    System.out.println("prbc:"+pendingReplicationBlocksCount);
    System.out.println("urbc:"+underReplicatedBlocksCount);
    System.out.println("srbc:"+scheduledReplicationBlocksCount);
    */
    ...
    //Text.writeString(out, "CHECKMSG 1");
    neededReplications.write(out); 
    //Text.writeString(out, "CHECKMSG 2");
    if(pendingReplications == null){
      System.out.println("pendingReplications is null : skipping saving it");
      out.writeBoolean(false);
    }else{
      out.writeBoolean(true);
      pendingReplications.write(out);
    }
    //Text.writeString(out, "CHECKMSG 3");
    host2DataNodeMap.write(out);
    //Text.writeString(out, "CHECKMSG 4");
