...
public class ViewChangeRow{
    ...
    public ViewChangeRow(){
	smallestView = 0;
	lastRemoved = 0;
	viewChanges = new ViewChangeCertificate[BFT.Parameters.getOrderCount()];
	pendingAcks = new ViewChangeAck[BFT.Parameters.getOrderCount()];
    }
	...
	if (viewChanges[index] == null || 
	    ...
	    viewChanges[index] = 
		new ViewChangeCertificate(vc.getView(), amprimary);
 ...
 public void addConfirmView(ConfirmView cv, boolean amprimary){
	...
	if (viewChanges[index] == null ||
	    cv.getView() > viewChanges[index].getView())
	    viewChanges[index] = 
		new ViewChangeCertificate(cv.getView(), amprimary);
	...
	return (int)(sn % BFT.Parameters.getOrderCount());
    }
}
