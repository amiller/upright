...
public class Certificate{
    ...
    public Certificate(int replicas){
	this(replicas, false);
    }

    public Certificate(int replicas, boolean cp){
    ...
    synchronized public Prepare getPrepare(){
	return prepare;
    }
	...
	if (entryDigest == null){
	    entryDigest = new Digest(certEntry.getBytes());
	}
	return entryDigest;
    }
	...
	if (certEntry == null){
	    ...
	    // new next batch
	    nextBatch = new CommittedNextBatch(currentView, seqNo,
					       certEntry, takeCP,
					       obn.getMyOrderIndex());
	    obn.authenticateExecMacArrayMessage(nextBatch);
		  ...
		  && (nextBatch == null 
		      ...
		      || nextBatch.getView() < currentView)){
	    nextBatch = new TentativeNextBatch(currentView, seqNo,
					       certEntry, takeCP,
					       obn.getMyOrderIndex());
	    obn.authenticateExecMacArrayMessage(nextBatch);
		   ...
		   || nextBatch.getView() < currentView){
	    nextBatch = new SpeculativeNextBatch(currentView, seqNo,
						 certEntry, takeCP,
						 obn.getMyOrderIndex());
	    obn.authenticateExecMacArrayMessage(nextBatch);
	...
	HistoryDigest history = new HistoryDigest(p.getHistory(), commands, 
						  nondet, p.getCPHash());
	certEntry = new CertificateEntry(history, commands, nondet, p.getCPHash());
	seqNo = p.getSeqNo();
    ...
    public synchronized boolean addMyPrepare(Prepare p){
	if (prepare != null)
	    return false;
	if ( getHistory() == null ||
	      !getHistory().equals(p.getPrePrepareHash()))
	    // gc p
	    return false;
	...
	if (certEntry == null)
	    throw new RuntimeException("can only prepare a "+
				       "next batch");
	for (int i = 0; i < commitMacs.length; i++){
	    prepareMacs[i] = true;
	    prepareCount++;
...
// 	return count;
    ...
    final public static int getFastQuorumSize(){
	return BFT.order.Parameters.fastOrderQuorumSize();
    }

    final public static int getQuorumSize(){
	return  BFT.order.Parameters.largeOrderQuorumSize();
    }

    final public static int getSmallQuorumSize(){
	return  BFT.order.Parameters.smallOrderQuorumSize();
    }
