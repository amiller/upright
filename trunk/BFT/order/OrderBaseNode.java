...
// agreement
import BFT.order.messages.ForwardedRequest;
import BFT.order.messages.PrePrepare;
import BFT.order.messages.Prepare;
import BFT.order.messages.Commit;
import BFT.order.messages.RequestBatch;

// view change
import BFT.order.messages.ViewChange;
import BFT.order.messages.ViewChangeAck;
import BFT.order.messages.NewView;
import BFT.order.messages.MissingViewChange;
import BFT.order.messages.RelayViewChange;
import BFT.order.messages.MissingCP;
import BFT.order.messages.RelayCP;
import BFT.order.messages.MissingOps;
import BFT.order.messages.RelayOps;
import BFT.order.messages.ConfirmView;
import BFT.order.messages.StartView;



import BFT.order.statemanagement.ViewInfo;
import BFT.order.statemanagement.ViewChangeRow;
import BFT.order.statemanagement.ViewChangeCertificate;

// resolving inconsistency
import BFT.order.messages.OrderStatus;
...
// order node state
import BFT.order.statemanagement.FilteredQuorum;
import BFT.order.statemanagement.Certificate;
...
public class OrderBaseNode extends BaseNode{
	...
	filteredQuorums = 
	    new FilteredQuorum[BFT.Parameters.getNumberOfClients()];
	for (int i = 0;i < filteredQuorums.length; i++)
	    filteredQuorums[i] = 
		new FilteredQuorum(BFT.Parameters.smallFilterQuorumSize(),
				   BFT.Parameters.mediumFilterQuorumSize(),
				   BFT.Parameters.largeFilterQuorumSize(),
	...
	for (int i = 0; i < certificates.length; i++){
		...
		certificates[i][j] = 
		    new Certificate(BFT.Parameters.getOrderCount(), cp);
	    }
	}
	for (int i = 0; i < futurePs.length; i++){
	    futurePs[i] = new Prepare[BFT.Parameters.getOrderCount()];
	    futureCs[i] = new Commit[BFT.Parameters.getOrderCount()];
	}
	stateSnapshots = new CheckPointState[BFT.order.Parameters.maxPeriods];
	stateSnapshots[0] = 
	    new CheckPointState(BFT.Parameters.getNumberOfClients());
	workingState = new CheckPointState(stateSnapshots[0]);
	currentIndex = 0;
	baseIndex = 0;

	cpCert = new Quorum<CPTokenMessage>(BFT.Parameters.getExecutionCount(),
					    BFT.Parameters.smallExecutionQuorumSize(),
					    0);

	// exec retransmission backoffs
	lastLastExecCheck = new long[BFT.Parameters.getExecutionCount()];
	lastExecDelay = new int[BFT.Parameters.getExecutionCount()];
	lastExecLoadCP = new boolean[BFT.Parameters.getExecutionCount()];
	...
	lastClientRetrans = new long[BFT.Parameters.getNumberOfClients()];

	// active counting
	active = new boolean[BFT.Parameters.getNumberOfClients()];
	activeCount = 0;
	...
	// view changes
	changingView = false;
	viewChanges = new ViewChangeRow[BFT.Parameters.getOrderCount()];
	vcPPs = new PrePrepare[2*BFT.order.Parameters.checkPointInterval];
	vcCommits = new Commit[BFT.Parameters.getOrderCount()];
	nvCache = new NewView[viewChanges.length];
	for (int i = 0; i < viewChanges.length; i++)
	    viewChanges[i] = new ViewChangeRow();

	observed = new double[BFT.Parameters.getOrderCount()];
	for (int i = 0; i < observed.length; i++){
	    observed[i] = 100000.0;
	}
	thresh = observed[0];
	observed[getMyOrderIndex()] = 0;
	...
	try{
	    ...
	    String suffix2 = "_"+getMyOrderIndex() + suffix;
	    String nbsuffix = "_NB.LOG";
	    String nbsuffix2 = "_"+getMyOrderIndex() + nbsuffix;
	    ...
	    }else {suffix = suffix2; nbsuffix = nbsuffix2;}
	    long CP1=-1, CP2=-1;
	...
	try{
	    System.out.println("attempting to load "+cp1);
	    int length = (int) cp1.length();
	    FileInputStream fs = new FileInputStream(cp1);
	    byte tmp[] = new byte[length];
	    fs.read(tmp);
	    fs.close();
	    CheckPointState tmp1 = new CheckPointState(tmp);
	    System.out.println(tmp1);

	    fs = new FileInputStream(cp2);
	    length = (int) cp2.length();
	    tmp = new byte[length];
	    fs.read(tmp);
	    fs.close();
	    CheckPointState tmp2 = new CheckPointState(tmp);
	    System.out.println(tmp2);
	    
	    System.out.println("Need to convert a stream to a sequence of nextbatch messages");
	    
	}catch(Exception e){BFT.Debug.kill(e);}
    }
    ...
    **/
	...
	case MessageTags.FilteredRequest: 
	    ...
	    process((FilteredRequestCore)vmb); 
	...
	case MessageTags.LastExecuted: 
	    ...
	    Debug.profileStart("LAST_EXEC");
	    break;
	case MessageTags.CPLoaded: process((CPLoaded) vmb); break;
	...
	case MessageTags.Commit: 
	    ...
	    break;
	case MessageTags.ViewChange: process((ViewChange) vmb); break;
	case MessageTags.ViewChangeAck: process((ViewChangeAck) vmb); break;
	case MessageTags.NewView: process((NewView) vmb); break;
	case MessageTags.MissingViewChange: process((MissingViewChange) vmb); break;
	case MessageTags.ConfirmView: process((ConfirmView)vmb); break;
	case MessageTags.RelayViewChange: process((RelayViewChange) vmb); break;
	case MessageTags.OrderStatus: process((OrderStatus) vmb); break;
	case MessageTags.MissingOps: process((MissingOps) vmb); break;
	case MessageTags.RelayOps: process((RelayOps) vmb); break;
	case MessageTags.StartView: process((StartView) vmb); break;
	case MessageTags.ForwardedRequest: process((ForwardedRequest) vmb); break;
	case MessageTags.MissingCP: process((MissingCP) vmb); break;
	case MessageTags.RelayCP: process((RelayCP) vmb); break;
	default: Debug.kill( new RuntimeException("order ndoe does not handle message "+vmb.getTag()));
	}
	//System.out.println("FINIS " + (System.currentTimeMillis()-start));
    }

    public void handle(FilteredRequestCore fr){
	Debug.profileStart("HANDLE_FILT_REQ");
	process(fr);
	Debug.profileFinis("HANDLE_FILT_REQ");
    }
    
    public void handle(PrePrepare pp){
	Debug.profileStart("HANDLE_CLEAN_PP");
	processClean(pp);
	Debug.profileFinis("HANDLE_CLEAN_PP");
    }

    long batCount = 0;

    public void handle(RequestCore[] rc){
	//		if(!procFirstReq) {
	//			start = System.currentTimeMillis();
	//			procFirstReq = true;
	//		}

	Debug.profileStart("HANDLE_CLEAN_RC");
	processClean(rc);
	Debug.profileFinis("HANDLE_CLEAN_RC");
	//		count += rc.length;
	//		batCount++;
	//		if (batCount % 1000 == 0)
	//			System.out.println("COUNT " + ((count) / (System.currentTimeMillis() - start)*1000) + ":" + ((batCount) / (System.currentTimeMillis() - start)*1000));
    ...
    /**
       Process a request.  Receive teh request, do black magic that
       eventually results in the next batch being sent.

       when full replication is implemented, black magic includes
       running the agreement protocol.

       Verification takes the following steps:

       (look up aardvark decision tree and put into place)
    ...
    **/
	...
	//Debug.println("process request @ seqno "+getCurrentSequenceNumber() + " " + c.getSendingClient() + " " + c.getRequestId());

	if (req.getSender() != c.getSendingClient())
	    Debug.kill("should not get a client request "+
		       "from different clietn than the core");
	...
	if (!amIPrimary())
	    return;
	if(!validateOrderMacArrayMessage(req)) {
	    BFT.Debug.kill(new RuntimeException("bad validate"));
	}
	RequestCore c = req.getCore();
	if (c.getRequestId() <=
    ...
    protected void process(FilteredRequestCore req){
	//		if(!procFirstFiltReq) {
	//			filterStart = System.currentTimeMillis();
	//			procFirstFiltReq = true;
	//		}

	//	System.out.println("Process filtered request @ seqno "+
	//		   getCurrentSequenceNumber()+" from "+
	//		   req.getSendingReplica() +" for "+
	//		   req.getSendingClient() + " at "+
	//		   req.getRequestId());
	...
	//		filtercount++;
	//		if (filtercount % 1000 == 0)
	//			System.out.println("COUNT " + ((double)(filtercount)));
	//	System.out.println("got a filtered request");
	...
	//		filtersmall++;
	//		if (filtercount % 1000 == 0)
	//			System.out.println("SCOUNT " + ((double)(filtersmall) / filtercount));

	// if the quorum is large then process the "cleaned" requestcore
	if (fq.medium() && amIPrimary()){
	    //if(true) return;
	    rcQueue.addCleanWork(fq.getEntry());
	    //	    fq.clear();
	    //	    System.out.println("\tmedium value for proposal");
	    return;
	}
	//		filtermed++;
	//		if (filtercount % 1000 == 0)
	//			System.out.println("MCOUNT " + ((double)(filtermed) / filtercount));

	// if the quorum is really big, start breaking bones

	if (fq.large()){
	    long lastOrdered = 
		getWorkingState().getLastOrdered(rc.getSendingClient());
	    if (!amIPrimary() || lastOrdered < rc.getRequestId())
		rcQueue.addCleanWork(fq.getEntry());
	    fq.clear();
	    //	    System.out.println("\tit was large");
	    if(true) return;
	}
	//		filterlarge++;
	//		if (filtercount % 1000 == 0)
	//			System.out.println("LCOUNT " + ((double)(filterlarge) / filtercount));
	...
	if (lastOrdered >= c.getRequestId()){
	    ...
	    Retransmit ret = new Retransmit(client,
	    ...
	    //			System.out.println("sending a retransmit");
	    System.out.println("retransmitting: "+ getCurrentSequenceNumber()+
	    		       " for client "+client); 
		...
		if (cert.getPrepare() == null){
		    ...
		    Prepare p = new Prepare(currentView, 
					    cs,
					    cert.getHistory(),
					    getMyOrderIndex());
		    authenticateOrderMacArrayMessage(p);
		    if (!changingView && amIPrimary())
			sendToAllOrderReplicas(p.getBytes());
		}
	    }
	    return true;
	}else{
    ...
    /**
       ...
       to be addressed
    **/
    protected void processClean(RequestCore[] c){
	//if(true) return;
	//Debug.println("process clean request core");

	if(c.length == 0)
	    BFT.Debug.kill("core size of 0?");

	// (2) If I am not the primary then forward to the primary
	if (!amIPrimary()) {
	    if (BFT.Parameters.filtered)
		return;
	    // exactly once semantics is fucked -- cant use this right now
	    //Debug.kill(new RuntimeException("should not have made it here"));
	    if (true)
		return;
	    // dont want to deal with forwarded requests yet
	    for(int i = 0; i < c.length; i++) {
		ForwardedRequest fr = 
		    new ForwardedRequest(getMyOrderIndex(), 
					 (SignedRequestCore)c[i]);
	    ...
	    // (3) I am the primary, so try to  create a preprepare
	    //System.out.println("pre nb");
	    createNextBatch(c);
	    //System.out.println("pst nb");
	}
    ...
    /**
       ...
       ** NOTE ** should only happen at start up
       ...
       (6)  add the preprepare to the next certificate
       (7)  send the preprepare to the other order replicas
       (8) create the next batch
       (9) add the next batch to the WorkingState and appropriate
       certificate
       (10)  if prepared then act on prepared

    **/
    long nonDetcounter = System.currentTimeMillis();
    protected void createNextBatch(RequestCore[] cores){
	long seqno = getCurrentSequenceNumber();
	...
	boolean cp = 
	    (getCurrentSequenceNumber()+1) % BFT.order.Parameters.checkPointInterval == 0;
	...
	if (!stateSnapshots[baseIndex].isStable()) 
		...
		Debug.kill( new RuntimeException("base should "+
						 "*always* be stable"));
	...
	RequestBatch rb = new RequestBatch(cores);
	...
	PrePrepare pp = 
	    new PrePrepare(currentView, seqno,
	...
	//  (6) add the preprepare to the apporpriate certificate
	if (!certificates[state][cert].addMyPrePrepare(pp))
	    Debug.kill("WTF");

	// (7) send the preprepare
	//	System.out.println("Sending PP: "+pp.getView()+" "+pp.getSeqNo());
	sendToOtherOrderReplicas(pp.getBytes());

	// (7a) see if a prepare is appropriate
	Certificate c = certificates[state][cert];
	if (primaryShouldPrepare()){
	    Prepare p = new Prepare(pp.getView(), pp.getSeqNo(),
				    c.getHistory(),
				    getMyOrderIndex());
	    authenticateOrderMacArrayMessage(p);
	    sendToAllOrderReplicas(p.getBytes());
	...
	// (8) create the next batch
	NextBatch nb = 
	    certificates[state][cert].getNextBatchMessage(this, currentView);
	authenticateExecMacArrayMessage(nb);
	// (9)  add the next batch to the agreement state
	addNextBatch(nb);
	// if its not a checkpoint interval rely on speculation
	if (!cp)
	    //sendToAllExecutionReplicas(nb.getBytes());
	    writeAndSend(nb);


	// (10) if the sequence number is prepared then act on the
	// prepared
	if (certificates[state][cert].isPrepared()){
	    //	    System.out.println("acton prepared in createnextabatch?");
	    actOnPrepared(pp.getSeqNo());
	}
	updateHeartBeat();
	...
	// 	    && getCurrentSequenceNumber() % (CPNOW-getMyOrderIndex()) == 0){
    ...
    **/
    protected void process(PrePrepare pp){
// 	Debug.println("\tprocess preprepare: "+pp.getSeqNo()+"/"+
// 	getCurrentSequenceNumber()+":"+
// 		      getBaseSequenceNumber()+" view: "+pp.getView());

	if ( pp.getSender() == getMyOrderIndex())
	    Debug.kill("WTF");
	...
	//if (pp.getSendingReplica() == getMyOrderIndex())
	//Debug.kill("WTF");

	if (pp.getSendingReplica() != 
	    primary(pp.getView()) % BFT.Parameters.getOrderCount())
	...
	if (pp.getSeqNo() != getCurrentSequenceNumber()){
	    ...
	    //Debug.println("Cached for near future "+pp.getSeqNo()+" "+
	...
	// (5) check the mac
	if (!validateOrderMacArrayMessage(pp) 
	...
	// (9) check the signatures on the request cores
	//Debug.println("\tsending preprepare to cleaner: "+pp.getSeqNo()+"/"+
	//		getCurrentSequenceNumber()+":"+
	//		getBaseSequenceNumber());
	if (!BFT.Parameters.filtered)
	    cleaner.clean(pp);
    ...
    **/
    protected void processClean(PrePrepare pp){
	...
	// (1) Consruct the next batch from the preprepare
	...
	if (cp){
	    //Debug.println("\tConstructing a Prepare for: "+pp.getSeqNo());
	    // (3) construct the prepare
	    ...
	    cert.addMyPrepare(p);
	    // (5)  send it out
	    sendToOtherOrderReplicas(p.getBytes());
	    ...
	    //Debug.println(Debug.COMMIT, "Sending spec next batch"+nb.getSeqNo());
	    //sendToAllExecutionReplicas(nb.getBytes());
	    writeAndSend(nb);
	...
	if (doViewChangeNow && 
	    ...
	    startViewChange(currentView+1);
		 ...
		 && getCurrentSequenceNumber() > 2*BFT.order.Parameters.checkPointInterval
		 && getCurrentSequenceNumber() % (CPNOW+getMyOrderIndex()) == 0
		 //+(BFT.order.Parameters.checkPointInterval+1) * getMyOrderIndex()) == 0 ){
		 ){
	    startViewChange(currentView+1);
	    return;
	}
    ...
    **/
    protected void process(Prepare p){
	//	Debug.println("\t\tProcessing Prepare "+p.getSeqNo()+" from: "+p.getSendingReplica());
	...
	if (seqno >= getBaseSequenceNumber() + 
	    ...
	    }else// (4a) if its distant future discard it
	...
	if (cert.preparedBy((int)p.getSendingReplica()) 
		...
		if (!validateOrderMacArrayMessage(p))
		...
		if (p.getSendingReplica() == primary(currentView)
		    && p.getSendingReplica() != getMyOrderIndex()){
		    if (cert.getPrepare() == null){
			//		//System.out.println("prepare (a)");
			Prepare p2 = new Prepare(p.getView(), p.getSeqNo(),
						 p.getPrePrepareHash(),
						 getMyOrderIndex());
			authenticateOrderMacArrayMessage(p2);
			sendToOtherOrderReplicas(p2.getBytes());
			added = cert.addMyPrepare(p2);
		    }
		}

		if (p.getSendingReplica() == getMyOrderIndex())
    ...
    **/
    protected void actOnPrepared(long seqNo){
	...
	Commit c = new Commit(currentView(), seqNo, cert.getHistory(),
			      getMyOrderIndex());
	...
	// (4) send it out
	sendToOtherOrderReplicas(c.getBytes());
    ...
    **/
    protected void process(Commit c){
	// Debug.println("\t\t\tProcessing Commit "+c.getSeqNo()+
	...
	if (!validateOrderMacArrayMessage(c))
    ...
    **/
    protected void actOnCommitted(long seqNo){
	...
	authenticateExecMacArrayMessage(nb);
	//Debug.println("sending next batch: "+nb.getSeqNo());
...
// 			       "\n     batch size of : "+size);
    ...
    **/
    protected void process(CPTokenMessage cptok){
	...
	if (!validateExecMacArrayMessage(cptok))
	    Debug.kill("invalid macarray");//return;
	...
	if (tmp != null && !tmp.matches(cptok) 
		...
		RequestCP rcp = new RequestCP(getBaseSequenceNumber(),
					      getMyOrderIndex());
    ...
    **/
    ...
    protected void makeCPStable(CheckPointState cps){
	...
	if(BFT.Parameters.doLogging) {
	    cps.markingStable();
	    CPLogger cpl = new CPLogger(cpwq, cps, getMyOrderIndex());
	    pool.execute(cpl);
	}
	else {
	    cps.markingStable();
    ...
    public void cpStable(CheckPointState cp){
...
// 	    System.out.println(cp);
	 ...
	 **/
	protected void addNextBatch(NextBatch nb){
		//Debug.println("adding next batch: "+nb.getSeqNo());
		...
		// 	int stateindex = stateIndex(nb.getSeqNo());
		// 	int certindex = certificateIndex(nb.getSeqNo());
    ...
    protected void newWorkingState(){
...
// 	Debug.println("\t\t\t\tBaseindex: "+baseIndex+" curIndex: "+currentIndex);
    ...
    **/
    protected void garbageCollect(long seqno){
	if (seqno >= getCurrentSequenceNumber())
	...
	while (getBaseSequenceNumber() < seqno && 
		...
		certificates[baseIndex][i] = 
		    new Certificate(BFT.Parameters.getOrderCount(), 
				    (i==BFT.order.Parameters.checkPointInterval-1)?true:false);

	    }
	    CheckPointState cps = stateSnapshots[baseIndex];

	    ReleaseCP rcp = new ReleaseCP(cps.getExecCPToken(),
					  cps.getCurrentSequenceNumber(),
					  getMyOrderIndex());
	    authenticateExecMacArrayMessage(rcp);
	    ...
	    if(!cpfile.exists()) {
		cpfile = new File(cps.getCurrentSequenceNumber() + "_" + getMyOrderIndex() + "_ORDER_CP.LOG");
	    }
	    if(cpfile.exists()) {
		cpfile.delete();
...
// 		System.out.println("heartbeat triggered batch!");
	...
	// unpile the pp and comm
	    ...
	    int bias = 4+BFT.Parameters.getFilterCount();
	...
	if(!changingView && !amIPrimary() &&
	   ...
	   System.currentTimeMillis() - heartBeat > BFT.order.Parameters.heartBeat){
     ...
     **/
    protected void startViewChange(long newView){
	if (DISABLE_VC)
	    return;

// 	Debug.println(Debug.VIEWCHANGE, 
// 		      "\t\t\t\t\t\tStarting view change for "+newView);
	...
	ViewChange vc = new ViewChange(currentView, 
				       cpSeqno,
				       stableCPSeqno,
				       prepareSeqno,
				       preprepareSeqno,
	...
	sendToOtherOrderReplicas(vc.getBytes());

	// create the new view information
	vInfo = new ViewInfo(currentView, vcCerts, 
			     primary(currentView) == getMyOrderIndex());
	changingView = true;
	vInfo.observeViewChange(vc);
	vInfo.setMyViewChange(vc);

	// create a vcack for my vc
	Digest d = new Digest(vc.getBytes());
	ViewChangeAck vcack = new ViewChangeAck(vc.getView(), 
    ...
    protected void process(ViewChange vc){
// 	Debug.println(Debug.VIEWCHANGE, 
...
// 		      " from "+vc.getSendingReplica());
	...
	if (!validateOrderMacArrayMessage(vc))
	...
	ViewChangeAck vcack = new ViewChangeAck(vc.getView(), 
						...
						d, getMyOrderIndex());
	authenticateOrderMacArrayMessage(vcack);
	if (primary(vc.getView()) != getMyOrderIndex())
	...
	if (currentView == vc.getView()){
	    ...
	    if (primary(currentView()) == getMyOrderIndex()){
		vInfo.addViewChangeAck(vcack);
	    ...
	    vcr.observeViewChange(vc, 
				  primary(vc.getView()) == getMyOrderIndex());
	    // if im the primary record the ack
	    if (primary(vc.getView()) == getMyOrderIndex())
    ...
    protected void process(ViewChangeAck vcack){
// 	Debug.println(Debug.VIEWCHANGE, 
...
// 			" from "+vcack.getSendingReplica());
	...
	if (vcack.getView() < currentView)
	    return;
	else if (primary(vcack.getView()) != getMyOrderIndex())
	    Debug.kill("wrong view");//return; // only the primary needs vcacks
	else if (!validateOrderMacArrayMessage(vcack))
    ...
    protected void process(NewView nv){
// 	Debug.println(Debug.VIEWCHANGE, 
...
// 			" from "+nv.getSendingReplica());
	...
	if (!validateOrderMacArrayMessage(nv))
	    Debug.kill("invalid macarray");//return;
    ...
    **/
    ...
    protected void actOnDefinedView(){
...
// 		      vInfo.getNextSeqNo());
	...
	if (committed == null 
	    ...
	    if (!requestedThings){
		MissingCP mcp = new MissingCP(vInfo.getBaseSequenceNumber(),
					      vInfo.getCommittedCPDigest(),
					      getMyOrderIndex());
		authenticateOrderMacArrayMessage(mcp);
		if (amIPrimary())
		    sendToOtherOrderReplicas(mcp.getBytes());
		else
...
// 				   stable.getBaseSequenceNumber());
		...
		// 		System.out.println("requesting cp token from exec: "+stable.getCurrentSequenceNumber());
 		RequestCP rcp = new RequestCP(stable.getCurrentSequenceNumber(),
 					      getMyOrderIndex());
 		authenticateExecMacArrayMessage(rcp);
 		sendToAllExecutionReplicas(rcp.getBytes());
		if (vInfo.getStableCP() == null)
	...
	if (missingop > 0  && !requestedThings){
	    ...
	    MissingOps mo = new MissingOps(currentView, ops, getMyOrderIndex());;
	    authenticateOrderMacArrayMessage(mo);

	    if (amIPrimary(currentView)){
		sendToOtherOrderReplicas(mo.getBytes());
	    }else{
...
// 		    System.out.println(vInfo.getNextSeqNo() % BFT.order.Parameters.checkPointInterval != 0);
    ...
    **/
    protected void actOnCompleteView(){
	...
	if (vInfo.confirmedBy(getMyOrderIndex())){
	...
	if (amIPrimary()){
	    ...
	    sendToOtherOrderReplicas(nv.getBytes());
	}
	...
	for (long i = getBaseSequenceNumber(); 
	     ...
	     i++){
	    if (!certificates[stateIndex(i)][certificateIndex(i)].isPrepared())
		certificates[stateIndex(i)][certificateIndex(i)].forcePrepared();
	}
	...
	ConfirmView cv = new ConfirmView(currentView,
					 vInfo.getNextSeqNo(),
	...
	sendToOtherOrderReplicas(cv.getBytes());
    ...
    protected void process(ConfirmView cv){
// 	Debug.println(Debug.VIEWCHANGE,
...
// 			" at seqno: "+cv.getSeqNo());
	...
	if (!validateOrderMacArrayMessage(cv)){
    ...
    protected void actOnConfirmedView(){
...
// 	if (!amIPrimary()){
// 	    StartView sv = new StartView(currentView, 
// 					 getCurrentSequenceNumber(),
// 					 getHistory(), getMyOrderIndex());
// 	    authenticateOrderMacMessage(sv, primary(currentView));
// 	    sendToOrderReplica(sv.getBytes(), primary(currentView));
// 	}
    ...
    protected void process(StartView sv){
	Debug.println(Debug.VIEWCHANGE,
	...
	while (ind < getCurrentSequenceNumber()){
	    ...
	    //System.out.println("**sending pp "+pp.getSeqNo()+" to "+
    ...
    protected void process(MissingCP mcp){
	Debug.println(Debug.VIEWCHANGE,
	...
	if (!validateOrderMacArrayMessage(mcp))
    ...
    protected void process(RelayCP rcp){
	Debug.println(Debug.VIEWCHANGE,
...
// 	}
    ...
    protected void process(MissingViewChange mv){
	Debug.println(Debug.VIEWCHANGE, 
	...
	MacBytes[] macs = new MacBytes[BFT.Parameters.getOrderCount()];
	for (int i = 0; i < macs.length; i++)
	    macs[i] = new MacBytes();
	RelayViewChange rvc =
	    new RelayViewChange(vInfo.getViewChange(mv.getSendingReplica()),
				macs, getMyOrderIndex());
    ...
    protected void process(RelayViewChange rvc){
	Debug.println(Debug.VIEWCHANGE, 
    ...
    protected void process(MissingOps mo){
	Debug.println(Debug.VIEWCHANGE,
	...
	if (!validateOrderMacArrayMessage(mo))
    ...
    protected void process(RelayOps rops){
    ...
    **/
	...
	Debug.println("process last executed "+last.getSequenceNumber()+" from "+last.getSendingReplica());
	Debug.println("current seqno: "+getCurrentSequenceNumber()+" base: "+getBaseSequenceNumber());
	...
	if (!validateExecMacArrayMessage(last))
	    Debug.kill("invalid macarray");//return;
	...
	if (!validateExecMacArrayMessage(cpl))
	    Debug.kill("invalid macarray");//return;
	...
	if (baseline < getCurrentSequenceNumber()){
		...
		for(int i = certificateIndex(baseline); 
		    ...
		    sendToExecutionReplica(nb.getBytes(), 
					   (int)sendingReplica);
		    //		    System.out.println("resending "+nb.getSeqNo()+" to "+sendingReplica);
		}
    ...
    **/
	...
	return (int) (viewNo % BFT.Parameters.getOrderCount());
    }
	...
	return primary(view) == getMyOrderIndex();
    }
	...
	OrderWorkQueue owq = new OrderWorkQueue();
	osn.setNBQueue(owq);
	osn.setCpwq(owq);
	osn.setCleanRCQueue(owq);
	...
	Worker w = new Worker(owq, owq, osn);
	Thread wt = new Thread(w);
	// nextbatch logging needs the exec network
	NBLogger l1 = new NBLogger(0, owq, execNet);
	NBLogger l2 = new NBLogger(1, owq, execNet);
	...
	Cleaner c = new Cleaner(cwq, owq, osn.getMembership());
	osn.setCleaner(c);
	CleanerDispatcher cd = new CleanerDispatcher(c, cwq, osn);
	Thread ct = new Thread(cd);
    ...
    public void setCleanRCQueue(RequestCoreQueue rc){
	rcQueue = rc;
    }
