...
/**
   WatchReply sent to a client after its request has been executed
 **/
public class WatchReply extends MacMessage{
     ...
     **/
    public WatchReply(long execReplica, long sequence, 
		   byte[] com){
     ...
     **/
    public WatchReply(byte[] bytes){
	super(bytes);
	if (getTag() != MessageTags.WatchReply)
    ...
    /**
       Total size of the WatchReply message based on the definition in
       WatchReply.hh and verifiable_msg.hh
     **/
	...
	return MessageTags.WatchReply;
    }

    public boolean equals(WatchReply rep){
...
// 	}
	...
	for (int i = 0; i < 8; i++)
	    tmp[i] = (byte)i;
	WatchReply vmb = 
	    new WatchReply(1,0,tmp);
	//System.out.println("initial: "+vmb.toString());
	UnsignedTypes.printBytes(vmb.getBytes());
	WatchReply vmb2 = 
	    new WatchReply(vmb.getBytes());
	...
	vmb = new WatchReply(134,8, tmp);
	//System.out.println("initial: "+vmb.toString());
	UnsignedTypes.printBytes(vmb.getBytes());
	 vmb2 = new WatchReply(vmb.getBytes());
	//System.out.println("\nsecondary: "+vmb2.toString());
	UnsignedTypes.printBytes(vmb2.getBytes());
