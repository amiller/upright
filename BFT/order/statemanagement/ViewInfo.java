...
public class ViewInfo{
    ...
    public ViewInfo(long view, ViewChangeCertificate[] vcerts,
		    boolean prim){

	this.primary = prim;
	viewNo = view;
	viewChanges = vcerts;
	if (vcerts.length != BFT.Parameters.getOrderCount())
	...
	for (int i = 0; i < viewChanges.length; i++){
		...
		viewChanges[i] = new ViewChangeCertificate(view, primary);
	    }
	    pendingAcks[i] = new ViewChangeAck[vcerts.length];
	}
	
	viewConfirmations = 
	    new Quorum<ConfirmView>(BFT.Parameters.getOrderCount(),
	...
	cvCache = new ConfirmView[BFT.Parameters.getOrderCount()];
	...
	for (int i = 0; i < certs.length; i++){
		...
		certs[i][j] = 
		    new Certificate(BFT.Parameters.getOrderCount(), takecp);
		histories[i][j] = null;
	...
	if (viewChanges[index]!=null){
		...
		|| (newViewMsg.getChosenVCs()[index] != null
		    && newViewMsg.getChosenVCs()[index].equals(new Digest(vc.getBytes())))){
...
// 		    >= BFT.Parameters.smallOrderQuorumSize())){
// 		root = tmproot;
...
// 		BFT.Parameters.smallOrderQuorumSize()){
// 		node = tmpnode;
// 		if (ppSeqNo +1 != node.getSeqNo())
// 		    Debug.kill("out of order in the batch list?");
// 		ppSeqNo = node.getSeqNo();
...
// 	for(int i = 0;
// 	    i < vcd.length && count < BFT.Parameters.largeOrderQuorumSize();
// 	    i++){
...
// 	} // for
	...
	newViewMsg = new NewView(viewNo, vcd, obn.getMyOrderIndex(),
				 BFT.Parameters.largeOrderQuorumSize());
...
// 		System.out.println(cps.getDigest());
	    ...
	    if (cps.getCurrentSequenceNumber() == 
		...
		if (stableCP == null 
		    ...
		    //		    System.out.println("success!");
		    return true;
		}else if (stableCP == null)		    System.out.println("stable cp not added");
	    }
	...
	res = stableCPDigest == null 
	    || (stableCP != null && stableCP.isStable());
	if (!res){
	...
	if (workingState.getCurrentSequenceNumber() !=
	    getNextSeqNo())
...
//	defineView(root, subroot);
	...
	if (primary
	    || (newViewMsg != null
		&& new Digest(vc.getBytes()).equals(newViewMsg.getChosenVCs()[(int)(vc.getSendingReplica())]))){
		...
		if (root.matchesCommitted(vc)){
		    ...
		    root.addStableSupport((int)
					  (vc.getSendingReplica()));
		    TreeNode node=null;
		    Digest p[] = vc.getBatchHashes();
		    HistoryDigest h[] = vc.getHistories();
		    int k = (int) (vc.getStableCPSeqNo() - 
				   vc.getCommittedCPSeqNo());
		    int send = (int) (vc.getSendingReplica());
		    if (p.length > 0
			&& vc.getPPSeqNo() >= root.getSequenceNumber()){
			node = root.getChild(p[k], h[k]);
		...
		if (vc.getStableCPSeqNo() == vc.getCommittedCPSeqNo())
		    ...
		    trees.add(new TreeRoot(vc, true));
	       
		else{
		    // add both roots
		    trees.add(new TreeRoot(vc, true));
		    trees.add(new TreeRoot(vc, false));
		}
		populateVCTrees(vc);
	    }else if (!com){
	    ...
	    else Debug.kill(new RuntimeException("OOPS"));
	}else
	    ;
