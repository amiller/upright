...
public class ViewChangeCertificate{
    ...
    public ViewChangeCertificate(long view, boolean primary){
	viewNumber = view;
	vc = null;
	vca = new ViewChangeAck[BFT.Parameters.getOrderCount()];
	...
	if (vc2.getView() != viewNumber)
	    throw new RuntimeException("cant add vc for "+vc2.getView()+
				       " to cert for "+viewNumber);
	Digest tmp = new Digest(vc2.getBytes());
	...
	if (vcack.getView() != viewNumber)
	    throw new RuntimeException("cant add vc for "+vcack.getView()+
				       " to cert for "+viewNumber);
	if (vcDigest == null){
	...
	for (int i = 0;i < vca.length; i++)
	    if (vca[i]!=null)
		count++;
	hasAcks = vc != null && count >= BFT.Parameters.largeOrderQuorumSize();
	return hasAcks;
    }
